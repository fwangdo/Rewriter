...
**Claude 메모 반영: extraction cost / smollm 상태**

- `egglog.extract()`를 cost model 없이 호출하면 의미 있는 latency extraction이 아니다.
  - default extraction은 AST/node-size 계열 기준으로 볼 수 있다.
  - 우리의 목표는 ORT latency가 낮은 graph 선택이므로 `greedy_dag_cost_model(...)`
    같은 cost-aware extraction 경로를 반드시 써야 한다.
- Python callback cost model 자체를 빼는 것은 방향이 아니다.
  - 문제가 callback overhead라면 callback을 최적화해야 한다.
  - 권장 방향: `_load_ir()` 시점에 `(op, attrs_key) -> cost`를 미리 계산하고,
    extraction callback에서는 `get_callable_args()`/`get_literal_value()`/`ENode`
    재구성을 반복하지 않고 dict lookup 중심으로 만든다.
- 현재 로컬 `egglog.py`에는 `extract_best(cost_model)`에서
  `greedy_dag_cost_model(cost_fn)`를 쓰려는 변경이 보인다.
  - 단, `_build_cost_fn` 구현 여부와 실제 동작은 아직 검증 필요하다.
  - `extract_topk(k>1)`는 여전히 `extract_multiple(root, k)`를 직접 쓰고,
    결과 dedup도 `repr(ir)` 기반이다.
- `repr(ir)` 기반 dedup은 큰 IR에서 비싸다.
  - ONNX materialization 후 stable graph hash를 쓰거나,
    IR node/op/attrs/input/output signature 기반의 lightweight hash로 바꿔야 한다.
- `smollm_135m` 현재 관찰:
  - `k=1` 후보가 correctness FAIL.
  - `bridge_only`에서도 FAIL, max_diff 약 `1.39e-03`.
  - `k=5`는 extraction/dedup 비용 때문에 timeout 성격의 문제가 있다.
  - 따라서 smollm 문제는 단순히 egglog pure rewrite 문제가 아니라,
    legacy bridge에서 materialize되는 rule 중 correctness drift를 만드는 것이
    있을 가능성이 높다.
- smollm의 다음 디버깅 방향:
  - `max_iter=0`, bridge on/off, rule family on/off를 나눠서 drift 발생 지점을 찾는다.
  - 특히 `MatMul -> Conv`, `Gemm` decomposition, shape/value synthesis rule을 먼저 본다.
  - correctness를 깨는 후보를 top-k로 많이 뽑는 상태에서는 k를 늘리는 것이
    latency 최적화로 이어지지 않는다.

**다음 우선순위**

[ ] `extract_best(cost_model)`의 `greedy_dag_cost_model` 적용을 완성/검증한다.
[ ] cost callback을 precomputed `(op, attrs_key) -> cost` lookup 중심으로 바꾼다.
[ ] `extract_topk(k>1)`도 cost-aware 후보 생성 경로를 쓰도록 바꾼다.
[ ] top-k dedup에서 `repr(ir)`를 제거하고 lightweight/stable graph hash로 교체한다.
[ ] smollm은 rule family bisect로 bridge-only correctness drift 원인을 먼저 찾는다.
